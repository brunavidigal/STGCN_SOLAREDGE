...
from scripts_aux.utils.math_utils import evaluation
...
def multi_pred(sess, y_pred, seq, batch_size, n_his, n_pred, step_idx, dynamic_batch=True):
    ...
    """
    pred_list = []
    gen_batch_result = gen_batch(seq, min(batch_size, len(seq)), dynamic_batch=dynamic_batch)

    for i in gen_batch_result:
