...
def z_inverse(x, minimum, maximum):
    """
    The inverse of function z_score().
    :param x: np.ndarray, input to be recovered.
    :param minimum: float, the value of min.
    :param maximum: float, the value of max.
...
def evaluation(y, y_, x_stats):
    ...
    if dim == 3:
        # single_step case
        v = z_inverse(y, x_stats['min'], x_stats['max'])
        v_ = z_inverse(y_, x_stats['min'], x_stats['max'])
